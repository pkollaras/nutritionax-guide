  ...
  const handlePdfUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      reader.onload = async (e) => {
        ...
        const { data, error } = await supabase.functions.invoke('parse-guidelines-pdf', {
          body: { pdfBase64: base64, userId: selectedUserId }
        });
          ...
          {selectedUserId && (
            <>
              <div className="flex gap-2">
                ...
                />
                <Button
                  onClick={() => document.getElementById('guidelines-pdf-upload')?.click()}
                  disabled={uploading || !selectedUserId}
                  variant="outline"
                >
                  <Upload className="mr-2 h-4 w-4" />
                  {uploading ? 'Parsing PDF...' : 'Upload PDF Guidelines'}
                </Button>
              </div>
